  ...
  let getData = async () => {
    await axios
      ...
      .then(async (solutionRes) => {
        ...
        setValue("dev_contact", solutionRes.data.results.dev_contact);
      });
  };
  ...
  if (role !== "d" && role !== "g")
    return (
      <p className="text-danger pt-3 pt-md-4 pt-xl-5 pb-2 mb-3 ">
        Apenas um Desenvolvedor ou o Gestor tem permissão para cadastrar,
        modificar ou excluir uma Solução de Causa-Raiz
      </p>
    );
  else
    return (
      <>
        ...
        >
          ...
          <Modal
            id="confirm"
            body="Deseja modificar a Solução de Causa-Raiz?"
            submit
          />
        </form>
      </>
